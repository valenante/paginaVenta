...
export default function EtaAnalisis() {
              ...
              <table className="ka-table">
                ...
                <tbody>
                  {dy.porEstacion.map((e) => (
                    <tr key={e.estacion}>
                      <td style={{ fontWeight: 700 }}>{e.estacion}</td>
                      <td>{e.platos}</td>
  ...
  return (
    <div className="ka">
      ...
      {/* A) Insights */}
      {insights.length > 0 && (
        <div className="ka-insights">
          {insights.map((ins, i) => (
            <div key={i} className={`ka-insight ka-insight--${ins.tipo}`}>
              <span className="ka-insight__icon">{ins.icon}</span>
              <div className="ka-insight__body">
                <div className="ka-insight__msg">{ins.msg}</div>
                <div className="ka-insight__action">{ins.action}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            {bottleneck.estaciones.map((e, i) => {
              ...
              return (
                <div key={e.estacion} className={`ka-bar-row ${i === 0 ? "ka-bar-row--top" : ""}`}>
                  <span className="ka-bar__name">{e.estacion}</span>
          ...
          <div className="ka-peak-grid">
            {peak.porEstacion.filter((e) => e.pico && e.noPico).map((e) => (
              <div key={e.estacion} className="ka-peak-card">
                <div className="ka-peak-card__name">{e.estacion}</div>
                <div className="ka-peak-card__row">
      ...
      {/* F) Trends sparklines */}
      {trends?.porEstacion && Object.keys(trends.porEstacion).length > 0 && (
        <div className="ka-section">
          <h3 className="ka-section__title">Tendencia semanal por estación</h3>
          <div className="ka-trends-grid">
            {Object.entries(trends.porEstacion).map(([slug, weeks]) => {
              if (weeks.length < 2) return null;
              const last = weeks[weeks.length - 1]?.avgDurMin ?? 0;
              const prevWeek = weeks[weeks.length - 2]?.avgDurMin ?? 0;
              const weekDelta = prevWeek > 0 ? Math.round(((last - prevWeek) / prevWeek) * 100) : 0;
              const cls = weekDelta < -2 ? "down" : weekDelta > 2 ? "up" : "flat";
              return (
                <div key={slug} className="ka-trend-card">
                  <div className="ka-trend-card__head">
                    <span className="ka-trend-card__name">{slug}</span>
                    <span className={`ka-trend-card__delta ka-trend-card__delta--${cls}`}>
                      {cls === "down" ? "\u2193" : cls === "up" ? "\u2191" : "\u2194"} {Math.abs(weekDelta)}%
                    </span>
                  </div>
                  <ResponsiveContainer width="100%" height={60}>
                    <LineChart data={weeks} margin={{ top: 2, right: 4, left: 4, bottom: 2 }}>
                      <Line type="monotone" dataKey="avgDurMin" stroke={cls === "up" ? "#ef4444" : cls === "down" ? "#22c55e" : "#64748b"} strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              );
